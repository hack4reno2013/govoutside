(function($)
{
	...
	(function()
	{
		...
		{
			this.container.find('.rwmb-map-latitude').val(latLng.lat());
			this.container.find('.rwmb-map-longitude').val(latLng.lng());
		}
			...
			$('#' + addressField).autocomplete({
				source: function(request, response) {
				 ...
				 select: function(event, ui) {
					that.container.find('.rwmb-map-latitude').val(ui.item.latitude);
					that.container.find('.rwmb-map-longitude').val(ui.item.longitude);					var location = new window.google.maps.LatLng(ui.item.latitude, ui.item.longitude);
